...
int sem_criar(int iniciaval){
	...
	//cria semaforo e verificando se ja existe semaforo com a chave KEY.
	if((semid=semget((ftok(PAI, (key_t)KEY )),1,IPC_CREAT|IPC_EXCL|0666))==-1){
		if(semid=semget((ftok(PAI,(key_t)KEY )),1,0666)==-1){
			printf("%s\n",VERMELHO);
		...
		printf("Semaforo criado pelo processo pai - PID %d com id: %d\n",pai,semid);
		fprintf(arq_log,"Semaforo criado pelo processo pai - PID %d com id: %d.\n",pai,semid);
	}
