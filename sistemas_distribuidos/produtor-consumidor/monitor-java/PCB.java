...
public class PCB{
	...
	private static int numeroproducer=1;
	private static int numeroconsumer=1;
	...
	public static void main (String[] args) {
            ...
            try{
                    ...
                    numeroproducer = Integer.parseInt(args[1]);
                    numeroconsumer = Integer.parseInt(args[2]);
		...
		if(!fechar){
			buffer= new Buffer(TAM,numeroproducer,numeroconsumer);
			Entrada entrada = new Entrada(buffer,TAM,numeroproducer,numeroconsumer,infinito);
			entrada.start();
			try{
				entrada.join();
